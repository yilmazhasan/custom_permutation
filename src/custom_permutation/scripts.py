...
def custom_permutate_by_options(item_list, item_options=None):
    ...
    if item_options:
        ...
        the_choices = [id_list[:] for _ in range(slot_count)]
    ...
    for _ in range(iter_count):
        ...
        next_ids = [the_choices[j][next_one[j]] for j in range(len(the_choices))]
        current_ctr = Counter(next_ids)
        if freq_in_range(current_ctr, needed_freq_by_id):
